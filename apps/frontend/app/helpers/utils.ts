...
export const projectsPageSteps = [
  ...
  },
  {
    ...
    content:
      "Click here to read the instructions on how to contribute or watch a live ceremony",
  },
  {
...
export const singleProjectPageSteps = [
  ...
  },
  {
    ...
    content:
      "Here you can copy the command needed to contribute to this ceremony",
  },
  {
  ...
  },
  {
    target: ".circuitsView",
    content:
      "Click here to view the circuits for this ceremony and their live statistics",
  },
  {
  ...
  },
  {
    ...
    content:
      "Click here to download the final zKey for this circuit (only if the ceremony has been finalized)",
  },
];
 ...
 */
export const getContributionLocalFilePath = (
  completeFilename: string
): string => `${localPaths.contributions}/${completeFilename}`;
...
export const getBucketName = (
  ...
  ceremonyPostfix: string
): string => `${ceremonyPrefix}${ceremonyPostfix}`;
...
export const downloadCeremonyArtifact = async (
  ...
  while (true) {
    ...
    setStatus(
      `Downloading: (${((receivedLength / totalLength) * 100).toFixed(2)}%)`,
      true
    );
  }
 ...
 */
export const getContributionsCollectionPath = (
  ceremonyId: string,
  circuitId: string
): string =>
 ...
 */
export const getTimeoutsCollectionPath = (
  ceremonyId: string,
...
export const uploadParts = async (
  ...
  for (
    ...
    if (!!ceremonyId)
      await temporaryStoreCurrentContributionUploadedChunkData(
        ceremonyId,
        chunk
      );
  }
...
export const multiPartUpload = async (
  ...
  if (
    ...
    !!temporaryDataToResumeMultiPartUpload.uploadId
  ) {
    // Step (0.A).
    ...
    multiPartUploadId = await openMultiPartUpload(
      bucketName,
      objectKey,
      ceremonyId
    );
...
export const getContributionsValidityForContributor = async (
  ...
  for await (const circuit of circuits) {
    ...
    const contribution = isFinalizing
      ? circuitContributionsFromContributor.filter(
...
export const handleContributionValidity = async (
  ...
  const validContributions = contributionsWithValidity.filter(
    (contributionWithValidity: ContributionValidity) =>
  ...
  for await (const contributionWithValidity of contributionsWithValidity) {
    ...
    const matchedContributions = participantContributions.filter(
      (contribution: Contribution) =>
        contribution.doc === contributionWithValidity.contributionId
    );
    ...
    if (matchedContributions.length > 1)
      throw new Error(
        `Duplicated circuit contribution information. Please, contact the coordinator.`
      );
    ...
    if (!contributionDocument.data() || !circuitDocument.data())
      throw new Error(
        `Something went wrong when retrieving the data from the database`
      );
...
export const handleTweetGeneration = async (
  ...
  const tweetUrl = generateCustomUrlToTweetAboutParticipation(
    ceremonyTitle,
    gistUrl,
    false
  );
 ...
 */
export const getGithubProviderUserId = async (
  githubToken: string
): Promise<string | null> => {
 ...
 * @returns {boolean}
 */
export const checkIfUserContributed = async (
  ceremonyId: string
): Promise<boolean> => {
  ...
  for (const circuit of circuits) {
    const hasContributed = await getCircuitContributionsFromContributor(
      ceremonyId,
      circuit.id,
      user.uid
    );
