import {
  ...
  getDocumentById,
  getLatestUpdatesFromParticipant,
} from "./firebase";
import {
  Contribution,
...
export const contribute = async (
  ...
  try {
    ...
    const canParticipateToCeremony =
      await checkParticipantForCeremony(ceremonyId);
    ...
    if (!canParticipateToCeremony) {
      ...
      if (activeTimeouts.length > 0) {
        ...
        if (!activeTimeout.data) {
          setStatus(
            "There seems to be an error with the timeout, please try again or contact an administrator",
            false
          );
          return;
        }
        ...
        const { seconds, minutes, hours, days } =
          getSecondsMinutesHoursFromMillis(Number(endDate) - Date.now());

        setStatus(
          `You are timed out. Timeout will end in ${convertToDoubleDigits(days)}:${convertToDoubleDigits(hours)}:${convertToDoubleDigits(
            minutes
...
export const handleStartOrResumeContribution = async (
  ...
  const updatedParticipant = await getDocumentById(
    `ceremonies/${ceremonyId}/participants`,
    participant.id
  );
  if (!updatedParticipant.data()) {
    setStatus(
      "The participant document seems to not have any data, please try again.",
      false
    );
    return;
  }
  ...
  if (
    updatedParticipantData.contributionStep ===
    ParticipantContributionStep.DOWNLOADING
  ) {
    setStatus("Downloading zKey", true);
    blob = await downloadCeremonyArtifact(
      bucketName,
    ...
    updatedParticipantData = await getLatestUpdatesFromParticipant(
      ceremonyId,
      participant.id
    );
  }
  ...
  const unsubscribeToCeremonyCircuitListener = onSnapshot(
    ...
    async (changedCircuit: DocumentSnapshot) => {
      ...
      if (latestParticipantPositionInQueue === 1) {
        setStatus(
          `You are now the first in the queue, getting ready for contributing.`,
          true
        );
        ...
        setStatus(
          `You are at position ${latestParticipantPositionInQueue} in the queue`,
          false
        );
        setStatus(
...
export const listenToParticipantDocumentChanges = async (
  ...
  const ceremonyDodc = await getDocumentById(
  ...
  const unsubscribe = onSnapshot(
    participant.ref,
    async (changedParticipant: DocumentSnapshot) => {
      // Extract data.
      const {
      ...
      if (
        ...
        const resumingAfterTimeoutExpiration =
          prevStatus === ParticipantStatus.EXHUMED;
        ...
        if (
          ...
          setStatus(
            "Verifying might have not started if you are in this step. Please wait for a confirmation or timeout",
            false
          );
        }
        ...
        if (
          ...
          const res = await getLatestVerificationResult(
            ceremonyId,
            circuit.id,
            participant.id
          );
          setStatus(
            `Result of previous contribution - verified = ${res}`,
            false
          );
        }
        ...
        if (timeoutTriggeredWhileContributing) {
          ...
          if (activeTimeouts.length !== 1) {
            setStatus(
              "You are timed out, please reload the page and wait for the timeout to expire",
              false
            );
            unsubscribe();
          }
          ...
          if (!activeTimeout.data) {
            setStatus(
              "There seems to be an error with the timeout, please reload the page and try again or contact the coordinator",
              false
            );
            unsubscribe();
          }
          ...
          setStatus(
            `You are timed out. Timeout will end in ${convertToDoubleDigits(days)}:${convertToDoubleDigits(hours)}:${convertToDoubleDigits(
              minutes
                ...
                Please reload the page and try again once the timeout expires`,
            false
          );
        ...
        if (completedContribution || timeoutExpired) {
          ...
          if (!timeoutExpired) {
            ...
            setStatus(
              `Progressing to the next circuit at position #${nextCircuit.data.sequencePosition}`,
              true
            );
            await progressToNextCircuitForContribution(ceremonyId);
          } else {
            // resume
            setStatus(
              `Resuming with circuit  #${nextCircuit.data.sequencePosition}`,
              true
            );
        ...
        if (alreadyContributedToEveryCeremonyCircuit) {
          ...
          setStatus(
            `The latest contribution was verifed as ${res ? "valid" : "invalid"}`,
            false
          );
          ...
          const url = await handlePublicAttestation(
            circuits,
            ceremonyId,
            participant.id,
          ...
          setStatus(
            `You can share your attestation by clicking the button below`,
            false,
            url
          );
          unsubscribe();
