...
export const generateGetObjectPreSignedUrl = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.generateGetObjectPreSignedUrl
  );
 ...
 * @param ceremonyId <string> - the unique identifier of the ceremony.
 */
export const progressToNextCircuitForContribution = async (
  ceremonyId: string
): Promise<void> => {
  const cf = httpsCallable(
    firebaseFunctions,
...
export const progressToNextContributionStep = async (ceremonyId: string) => {
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.progressToNextContributionStep
  );
...
export const permanentlyStoreCurrentContributionTimeAndHash = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames
...
export const openMultiPartUpload = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.startMultiPartUpload
  );
      ...
      setTimeout(() => {
        const unsubscribeToCeremonyCircuitListener = onSnapshot(
          ...
          async (changedCircuit: DocumentSnapshot) => {
            ...
            // Extract data.
            const {
              avgTimings: changedAvgTimings,
              waitingQueue: changedWaitingQueue,
            } = changedCircuit.data()!;
            const {
            ...
            const {
              avgTimings: prevAvgTimings,
              waitingQueue: prevWaitingQueue,
            } = changedCircuit.data()!;
            const {
            ...
            const invalidContribution =
              prevFailedContributions === changedFailedContributions - 1;
            const validContribution =
              prevCompletedContributions === changedCompletedContributions - 1;
...
export const resumeContributionAfterTimeoutExpiration = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.resumeContributionAfterTimeoutExpiration
  );
...
export const generatePreSignedUrlsParts = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.generatePreSignedUrlsParts
  );
...
export const completeMultiPartUpload = async (
  ...
  const cf = httpsCallable(
    firebaseFunctions,
    commonTerms.cloudFunctionsNames.completeMultiPartUpload
  );
